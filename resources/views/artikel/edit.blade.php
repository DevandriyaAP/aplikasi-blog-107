                ...
                <form action="{{ route('artikel.update', $artikel->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('penulis_id') is-invalid @enderror" 
                            ...
                            @foreach($penulis as $p)
                                <option value="{{ $p->id }}" {{ old('penulis_id', $artikel->penulis_id) == $p->id ? 'selected' : '' }}>
                                    {{ $p->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('kategori_artikel_id') is-invalid @enderror" 
                            ...
                            @foreach($kategoris as $k)
                                <option value="{{ $k->id }}" {{ old('kategori_artikel_id', $artikel->kategori_artikel_id) == $k->id ? 'selected' : '' }}>
                                    {{ $k->nama_kategori }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('isi') is-invalid @enderror" 
                                  id="isi" name="isi" placeholder="Masukkan isi artikel" rows="8" required>{{ old('isi', $artikel->isi) }}</textarea>
                        @error('isi')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <small class="text-muted">Biarkan kosong jika tidak ingin mengganti gambar. Format: JPG, PNG, GIF (Max: 2MB)</small>
                        @error('gambar')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
